...
class OutStargramPostListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        (application as MasterApplication).service.getAllPosts().enqueue(
            object : retrofit2.Callback<ArrayList<OutStarPost>> {
                ...
                override fun onResponse(
                    ...
                    if(response.isSuccessful) {
                        ...
                        val adapter = OutstarPostAdapter(postList!!, LayoutInflater.from(this@OutStargramPostListActivity),glide)

                        val recyclerView = findViewById<RecyclerView>(R.id.outstar_recycleview)
                        recyclerView.adapter = adapter
                        recyclerView.layoutManager = LinearLayoutManager(this@OutStargramPostListActivity)
...
class OutstarPostAdapter(
    var postList: ArrayList<OutStarPost>
    ,val inflater: LayoutInflater
    ,val glide: RequestManager
): RecyclerView.Adapter<OutstarPostAdapter.ViewHolder>() {
